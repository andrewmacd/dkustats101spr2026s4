...
ggplot(mcd.bbpcf, aes(x=Calories, y=Category, fill=Category)) +
  geom_boxplot() + 
  guides(fill="none")
```
...
```{r}
#| label: tbl-outliers
#| tbl-cap: Calories of outlier items

mcd.outliers <- mcd.bbpcf %>% 
  filter(Calories > 900) %>% 
  select(Item, Category, Calories) %>% 
  arrange(Calories)
...
The others seem ok but it's worth excluding the Chicken McNuggests (40 piece) as that seems not meant for one person.
...
```{r}
#| label: tbl-carbmean
#| caption: Mean of Carbohydrates

mcdonalds %>% 
  summarise(Mean = round(mean(Carbohydrates), 2)) %>% 
  kbl() %>% 
  kable_styling()
```

Problem is outliers or non-normal distribution gives a misleading perception of the mean.

3.  Calculate and display the standard deviation of `Carbohydrates` using the `sd()` command.

```{r}
#| label: tbl-carbsd
#| caption: SD of Carbohydrates

mcdonalds %>% 
  summarise(SD = round(sd(Carbohydrates), 2)) %>% 
  kbl() %>% 
  kable_styling()
```
...
4.  Then calculate the number of `Carbohydrates` that are -2, -1, 0, 1, and 2 standard deviations above and below the mean. Make a table using the Table menu. ![](table.png){width="205"}
    ...
    | z score | Carbohydrates |
    |---------|---------------|
    | -2      |    `r round(mean(mcdonalds$Carbohydrates) + (sd(mcdonalds$Carbohydrates) * -2), 2)`          |
    | -1      |    `r round(mean(mcdonalds$Carbohydrates) + (sd(mcdonalds$Carbohydrates) * -1), 2)`           |
    | 0       |     `r round(mean(mcdonalds$Carbohydrates), 2)`           |
    | 1       |     `r round(mean(mcdonalds$Carbohydrates) + (sd(mcdonalds$Carbohydrates)), 2)`          |
    | 2       |     `r round(mean(mcdonalds$Carbohydrates) + (sd(mcdonalds$Carbohydrates) * 2), 2)`          |
